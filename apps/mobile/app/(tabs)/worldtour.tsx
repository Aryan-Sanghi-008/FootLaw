...
function RegionNode({ region, index }: { region: any; index: number }) {
  ...
  return (
    <View className="mb-8 relative">
       ...
       <TouchableOpacity 
         activeOpacity={0.9}
         className={`flex-row items-center p-lg rounded-[28px] border ${isActive ? 'bg-surfaceContainerHigh border-primary/50 shadow-xl shadow-primary/20' : 'bg-surfaceContainerLow border-white/5 opacity-60'}`}
       >
          <View className={`w-[90px] h-[90px] rounded-[24px] items-center justify-center ${isActive ? 'bg-primary/20' : 'bg-surfaceVariant'}`}>
             <Ionicons name={region.icon as any} size={40} color={isActive ? Colors.primary : Colors.outline} />
             {isActive && (
               <View className="absolute inset-0 rounded-[24px] border border-primary/30" />
             )}
          </View>
    ...
    <View className="flex-1 bg-background">
      <SafeAreaView className="flex-1" edges={['top']}>
        ...
        <ScrollView contentContainerStyle={{ padding: 20 }} showsVerticalScrollIndicator={false}>
          ...
          {/* Map Feature Card */}
          <View className="h-[280px] rounded-[28px] overflow-hidden border border-white/10 mb-xl relative shadow-2xl">
